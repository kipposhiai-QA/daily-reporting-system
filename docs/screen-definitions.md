...
| ユーザー切替   | 認証を実装しない代わりに、SALES_PERSON マスタから「誰として操作するか」を選択するドロップダウン。選択中のユーザーの `is_manager` により、閲覧・操作できる範囲が変わる（0.2参照）。選択状態はブラウザ内に保持する |

初期状態（ブラウザ内に選択状態が無い、または保存済みIDが一覧に存在しない）の場合は、`SALES_PERSON`一覧の先頭ユーザーを自動選択する。選択を促す専用画面は設けない。
