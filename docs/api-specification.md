...
全APIエンドポイントは、Supabase Authのログインセッション（cookie）で「現在のユーザー」を識別する。サーバーはログイン中のSupabase Authユーザーを検証し、対応する `SALES_PERSON`（`auth_user_id` で紐付け）を「現在のユーザー」として解釈する。未ログインの場合は401 `UNAUTHENTICATED`。以降の権限判定（`is_manager` の確認、日報の所有者チェック等）にはこの「現在のユーザー」を用いる。

例外として `GET /api/sales-persons` のみ、未ログインでも呼び出し可能とする。

かつては `X-Sales-Person-Id` ヘッダーの値をサーバーが無条件に信頼しており、クライアントが任意の sales_person_id を指定して他人になりすませる問題があったが（Issue #78）、全エンドポイントをこの方式に移行することで解消した。

### 1.3 権限エラーの扱い

| コード             | HTTPステータス | 意味                                                                              |
| ------------------ | -------------- | --------------------------------------------------------------------------------- |
| `UNAUTHENTICATED`  | 401            | 未ログイン、またはログイン中のユーザーに対応する営業担当者が存在しない（1.2参照） |
...
全エンドポイントは、Supabase Authのログインセッション（cookie）で「現在のユーザー」を識別する（1.2参照）。未ログインの場合は401 `UNAUTHENTICATED`。
...
| フィールド     | 必須 | バリデーション          |
| -------------- | ---- | ----------------------- |
| company_name   | ○    | 1文字以上200文字以内    |
| contact_person | -    | 50文字以内              |
| phone          | -    | 20文字以内              |
| email          | -    | メール形式、254文字以内 |
| address        | -    | 200文字以内             |

**レスポンス 201**: 登録内容
**エラー**: 422（`company_name` 未入力・`email` 形式不正）
...
本章の全エンドポイントは、Supabase Authのログインセッション（cookie）で「現在のユーザー」を識別する（1.2参照）。未ログインの場合は401 `UNAUTHENTICATED`。
...
- ログイン中のユーザーが `is_manager = false`（営業）の場合:
  - 自分が作成した日報のみ返却（`status` は DRAFT / SUBMITTED 両方含む）
  - `sales_person_id` クエリは無視する
- ログイン中のユーザーが `is_manager = true`（上長）の場合:
...
作成者はログイン中のユーザーとして扱う。ボディでの `sales_person_id` 指定は不要かつ無視する。
...
| problem / plan | -                                                              | 各1000文字以内                                                                                         |
| visit_records  | status=DRAFTなら任意（0件可）、status=SUBMITTEDなら1件以上必須 | 各要素の `customer_id` と `visit_content` は必須（`visit_content` は500文字以内）。`visit_time` は任意 |
...
作成者本人のみ許可（ログイン中のユーザーが作成者と一致しない場合 403）。
...
コメント投稿者はログイン中のユーザーとして扱う。Supabase Authのログインセッション（cookie）で識別する（1.2参照）。

**アクセス可否**

- ログイン中のユーザーが `is_manager = false` の場合: 403
